  ...
  derived_table: {
    ...
    sql:
      SELECT
              user_id,
              event_timestamp,
              CASE WHEN TIMESTAMP_DIFF(event_timestamp, last_event,MINUTE) >= 20 --session timout = 20 minutes
                     OR last_event IS NULL
                   THEN 1 ELSE 0 END AS is_new_session
         FROM (
              SELECT user_id,
                     event_timestamp,
                     LAG(event_timestamp,1) OVER (PARTITION BY user_id ORDER BY event_timestamp) AS last_event
  ...
  derived_table: {
    ...
    sql: SELECT unique_session_id,
      player_session_sequence,
       ...
       TIMESTAMP_DIFF(MAX(event_timestamp), MIN(event_timestamp),MINUTE) AS session_length_minutes
  FROM (
SELECT user_id,
       event_timestamp,
       SUM(is_new_session) OVER (ORDER BY user_id, event_timestamp) AS unique_session_id,
       SUM(is_new_session) OVER (PARTITION BY user_id ORDER BY event_timestamp) AS player_session_sequence
